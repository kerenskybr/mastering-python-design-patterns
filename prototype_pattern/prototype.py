...
class Book:
    def __init__(self, name, authors, price, **rest):
        ''' rest: publisher, length, tags, publication, etc'''
        self.name = name
    ...
    def __str__(self):
        mylist = []
        oredered = OrderedDict(sorted(self.__dict__.items()))
        for i in oredered.keys():
            mylist.append('{}:{}'.format(i, oredered[i]))
            if i == 'price':
                mylist.append('$')
            mylist.append('\n')
        return ''.join(mylist)
...
def main():
    b1 = Book('The C Programming Language', ('Brian W. Kernighan',
        ...
        'programming', 'algorithms', 'data structures'), test='test')
    prototype = Prototype()
    cid = 'k&r-first'
